...
use crate::model::api::{GetThreadBoxesRequest, OpenMessageRequest};
...
impl Validatable for GetThreadBoxesRequest {
    fn validate(&self) -> Result<()> {
        ...
        self.public_key
            .as_ref()
            .ok_or_else(|| anyhow!("missing pub jey"))?;

        Ok(())
    }
}

impl crate::model::extensions::Signer for GetThreadBoxesRequest {
...
impl Signed for GetThreadBoxesRequest {
    fn verify_signature(&self) -> Result<()> {
        ...
        // print sha of message to compare with client...
        ...
        // info!("signature: {}", hex_string(signature.as_ref()));

        let signer_pub_key = ed25519_dalek::PublicKey::from_bytes(signer.key.as_ref())?;

        // info!("signer public key: {}", hex_string(signer_pub_key.as_ref()));
